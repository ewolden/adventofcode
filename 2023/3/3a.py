...
def has_adjacent(arr, line_idx, idx):
    if line_idx > 0:
        if (idx > 0 and is_symbol(arr[line_idx-1][idx-1])) \
            or (is_symbol(arr[line_idx-1][idx])) \
            or (idx < len(arr[0])-1 and is_symbol(arr[line_idx-1][idx+1])):
            return True
    if (idx > 0 and is_symbol(arr[line_idx][idx-1])) \
        or (idx < len(arr[0])-1 and is_symbol(arr[line_idx][idx+1])):
        return True
